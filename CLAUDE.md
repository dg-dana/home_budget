...
- **Nothing is queued; the candidates are at the top of `TODO.md`.** Self-service "forgot password" is **live as of deploy run #27** — `POST /auth/forgot` plus a `/forgot` page linked from the sign-in screen (`ARCHITECTURE.md` §4) — but **nobody has used it on the real site yet**, which is the only check that counts for anything that has to land in an inbox. Four things about it are load-bearing and none is a detail: the response is the **same 202 and the same wording whether or not the address has an account**, so the route cannot be used to find out who has one; the **link is never in the response**, only in the email; with **no `RESEND_API_KEY` the route refuses outright** (503, "ask a household owner") because showing the link on an unauthenticated route would open every account to anybody who knows its address; and it is limited **per address** (5/hour) as well as per IP, since each request mails somebody who did not ask. Owner-issued recovery still exists and is now the fallback rather than the only way.
