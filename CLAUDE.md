...
npm run test:e2e   # playwright, guest flow + statistics + households (21 tests)
...
- `npm run test:e2e` — `e2e/`, Playwright against the **production build** (it runs `npm run build && npm start` itself). Covers the guest flow — including adding an item with a comment, copying the list as text, and a member's open page catching up with a guest on its own — the statistics page, and the household switcher. It runs the server with `RATE_LIMITS=off`, which `config.ts` ignores in production.
