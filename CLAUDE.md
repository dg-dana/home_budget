...
npm test           # vitest, server integration suite (223 tests)
...
- **The next thing to build is at the top of `TODO.md`** — self-service "forgot password". Half of it is already live and public (`GET /auth/reset/:token`, `POST /auth/reset`, the `/reset/:token` page, and a `created_by` column that is already nullable, so no migration); what is missing is the *request* half, one unauthenticated route plus a link from the sign-in page. `TODO.md` lists the four decisions to make first, of which the load-bearing one is that the response must **never** reveal whether an address has an account.
