...
- **The "Danger zone" at the foot of `/household` deletes an account or the whole household** (`DELETE /auth/account`, `DELETE /household`). Both confirm with the caller's **password**, not just a dialog, and no migration was needed — the schema's `ON DELETE CASCADE` already did the work (`ARCHITECTURE.md` §3, "Leaving, and closing an account or a household"). Two things to know before touching it: the same "Delete my account" also sits on **`/households`**, because the Household page needs a household open and an account with none had no way out otherwise; and a household's **only** owner is refused while anyone else is still in it. Ownership is handed over with `PUT /household/members/:id/role` ("Make owner"), which refuses to change your *own* role — that is what keeps an owner in every household. Live since run #16.
- **You can leave a household without deleting your account** (`DELETE /household/members/me`, merged, **not yet deployed**). "Leave this household" is the first thing in the Danger zone on `/household`. Three things to know before touching it: the route is registered **before** `/members/:id`, which sits behind `requireOwner` — reversed, Express would 403 every ordinary member, which is the whole group it is for; it is the **only** thing in that card with no password field, deliberately, because a new invite undoes it (`ARCHITECTURE.md` §3); and it refuses the **only owner with company** and the **last person in the household** — the second because leaving would abandon the rows, where deleting an account takes an empty household with it. The owners are emailed, the leaver is not.
