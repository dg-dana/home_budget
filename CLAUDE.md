...
npm test           # vitest, server integration suite (197 tests)
...
- `npm test` — `server/test/`, real HTTP against a real SQLite DB. The one stub in the suite is `fetch` in `notifications.test.ts`, so nothing ever mails a real provider. Each test file gets its own database. **Always `npm test`, never `npx vitest` from the repo root** — the config lives in `server/`, so a root run skips `setupFiles` and every file silently shares one database, which looks like a flood of unrelated failures.
