...
npm test           # vitest, server integration suite (188 tests)
...
- **Nothing has ever sent an email, and that is now the blocker** — see `TODO.md`, which is the working brief for it. Invites, password recovery and the address confirmation added in run #17 all put a link **on screen** for whoever is signed in to pass on by hand. So "the invited person did not get an email" is expected, not a bug: the owner copies the link and sends it themselves. `server/src/notifications.ts` is the single seam — teach `deliver()` to send and confirmation becomes real with no caller changes; invites and recovery still need routing through it. **Resend is the chosen provider**, and `TODO.md` carries the step-by-step for both halves — the account, DNS records and API key only a human can create, then the code changes. Start there rather than re-deriving the plan. **Until then, do not describe confirmation as verification anywhere user-facing** (`ARCHITECTURE.md` §14).
